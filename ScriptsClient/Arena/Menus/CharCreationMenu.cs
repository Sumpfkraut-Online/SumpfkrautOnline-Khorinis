...
namespace GUC.Scripts.Arena
{
    class CharCreationMenu : GUCMainMenu
    {
        ...
        void ChangedVisual() //FIXME: Fleischzoepfe verhindern!
        {
            ...
            info.BodyMesh = (HumBodyMeshs)bodymesh.Choice;
            info.BodyTex = (HumBodyTexs)bodytex.Choice;
            info.HeadMesh = (HumHeadMeshs)headmesh.Choice;
            info.HeadTex = (HumHeadTexs)headtex.Choice;
            info.Fatness = fatness.Choice / 100.0f;
            info.BodyWidth = bodywidth.Choice / 100.0f;

            Character.SetAdditionalVisuals(info.BodyMesh.ToString(), (int)info.BodyTex, info.HeadMesh.ToString(), (int)info.HeadTex);
            Character.SetFatness(info.Fatness);
            Character.SetScale(new Types.Vec3f(info.BodyWidth, 1.0f, info.BodyWidth));
        }
