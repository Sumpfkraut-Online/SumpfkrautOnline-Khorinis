    ...
    class GUCTextBox : GUCView
    {
        ...
        public void KeyPressed(VirtualKeys key)
        {
            ...
            else
            {
                ...
                if (!AllowSymbols && InputHandler.AllChars.Keys.ToList().IndexOf(c) > 52)
                    return;


                if (fixedBorders && StringPixelWidth(Input) + InputHandler.AllChars[c] > width) //check if fixed borders are reached
                    return;
        ...
        string GetCharFromKey(VirtualKeys key)
        {
            ...
            if (Gothic.mClasses.InputHooked.IsPressed((int)VirtualKeys.Shift))
                keyboardState[(int)VirtualKeys.Shift] = 0xff;
            if (Gothic.mClasses.InputHooked.IsPressed((int)VirtualKeys.Control) && Gothic.mClasses.InputHooked.IsPressed((int)VirtualKeys.Menu))
            {
