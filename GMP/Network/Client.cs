...
namespace GUC.Client.Network
{
    ...
    static class Client
    {
        ...
        public static void Connect(String ip, ushort port, String password)
        {
            if (isConnected || nextConnectionTry > DateTime.UtcNow.Ticks)
                return;

            if (connectionTrys >= 10)
                Logger.LogError("Verbindung nicht möglich!");

            string pw = Constants.VERSION + password;
            if (clientInterface.Connect(ip, port, pw, pw.Length) != ConnectionAttemptResult.CONNECTION_ATTEMPT_STARTED)
            {
                Logger.LogError("Es konnte keine Verbindung aufgebaut werden.");
            }
