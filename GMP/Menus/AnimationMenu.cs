        ...
        bool ActivateItem(VirtualKeys key)
        {
            ...
            if (num == 10 && current.Parent != null)
            {
                if(current.StopAni != Animations.INVALID)
                {
                    ...
                    {
                        zERROR.GetZErr(Program.Process).Report(2, 'G', "Intern Stop Animation", 0, "hGame.cs", 0);
                        StopAnimation(current.StopAni);
                    }
                    else
                    {
                        zERROR.GetZErr(Program.Process).Report(2, 'G', "Intern Play Animation", 0, "hGame.cs", 0);
