...
namespace GUC.Sumpfkraut.Ingame
{
    ...
    class AnimationMenu : GUCInputReceiver
    {
        ...
        public static AnimationMenu GetMenu()
        {
            if (animMenu == null)
            {
                animMenu = new AnimationMenu();
            }
            return animMenu;
        }

        public VirtualKeys ActivationKey = VirtualKeys.X;
        ...
        public AnimationMenu()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                TextLines[i] = new GUCMenuText("", x + 5, y + 5 + offset * i, false);
                //TextLines[i].Show();
            }
        ...
        {
            if (key == (int)ActivationKey || key == (int)VirtualKeys.Escape)
            {
                background.Hide();
