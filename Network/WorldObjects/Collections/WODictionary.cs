...
{
    public class WODictionary<TWorldObject> : WOStorage<TWorldObject> where TWorldObject : WorldObject
    {
        List<int> freeIDs;
        int idCount;

        internal WODictionary(int maximum = WorldObject.MAX_ID) : base(maximum)
        {
        ...
        internal override void Add(TWorldObject obj, ref int idRef)
        {
            if (obj == null)
            ...
            else
            {
                if (idCount >= maximum)
                {
                    throw new Exception("WODictionary reached maximum! " + maximum);
                }
        ...
        #region Remove
        internal override bool Remove(TWorldObject obj, ref int idRef)
        {
            if (base.Remove(obj, ref idRef))
            {
                freeIDs.Add(idRef);
                idRef = -1;
            }
