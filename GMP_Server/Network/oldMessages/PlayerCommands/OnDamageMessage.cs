...
namespace GUC.Server.Network.Messages.PlayerCommands
{
    class OnDamageMessage : IMessage
    {
        ...
        public void Read(BitStream stream, Client client)
        {
            ...
            NPC vicProto = (NPC)sWorld.VobDict[victim];
            Vob attProto = null;
            ...
            Scripting.Objects.Character.NPC.isOnDamage(vicProto.ScriptingNPC, (DamageTypes)damageMode, locHit, flydir, attackerScriptProto, weaponMode, scriptSpell, weaponScriptItem, fallDownDistanceY);
            using (RakNetGUID guid = new RakNetGUID(client.guid))
            Write(vicProto, (DamageTypes)damageMode, locHit, flydir, attProto, weaponMode, spell, weaponIt, fallDownDistanceY, guid);
        }

        public static void Write(NPC victim, DamageTypes damageMode, Vec3f hitLoc, Vec3f flyDir, Vob attacker, int weaponMode, Spell spellID, Item weapon, float fallDownDistanceY, AddressOrGUID guidExclude)
        {
