    ...
    class EquipItemMessage : IMessage
    {
        public void Read(BitStream stream, Client client)
        {
            ...
            if (equip)
            {
                ...
                Scripting.Objects.Character.NPC.isOnEquip(player.ScriptingNPC, item.ScriptingProto);
            }
            else
            {
                if (item.ItemInstance.MainFlags.HasFlag(MainFlags.ITEM_KAT_ARMOR))
                ...
                Scripting.Objects.Character.NPC.isOnUnEquip(player.ScriptingNPC, item.ScriptingProto);
            }
