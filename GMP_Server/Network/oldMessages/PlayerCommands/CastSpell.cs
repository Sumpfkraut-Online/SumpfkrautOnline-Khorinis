...
namespace GUC.Server.Network.Messages.PlayerCommands
{
    class CastSpell : IMessage
    {
        public void Read(BitStream stream, Client client)
        {
            ...
            Scripting.Objects.Character.NPC.isOnCastSpell(
                caster.ScriptingNPC, spell.ScriptingProto, sT);


            using (RakNetGUID guid = new RakNetGUID(client.guid))
            Write(caster, item, target, spell, guid);
        }


        public static void Write(NPC proto, Item itm, Vob target, Spell spell)
        {
            Write(proto, itm, target, spell, null);
        }
        public static void Write(NPC proto, Item itm, Vob target, Spell spell, AddressOrGUID guidExclude)
        {
