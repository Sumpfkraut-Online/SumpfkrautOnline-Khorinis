...
namespace GUC.Server.Network.Messages.NpcCommands
{
    class AnimationUpdateMessage : IMessage
    {
        public void Read(BitStream stream, Client client)
        {
            ...
            short oldAnim = ((NPC)vob).Animation;
            ((NPC)vob).Animation = anim;

            using (RakNetGUID guid = new RakNetGUID(client.guid))
            Write((NPC)vob, guid);

            Scripting.Objects.Character.NPC.isOnAnimation(((NPC)vob).ScriptingNPC, anim, oldAnim);
        }
        ...
        public static void Write(NPC proto, AddressOrGUID addGuid)
        {
